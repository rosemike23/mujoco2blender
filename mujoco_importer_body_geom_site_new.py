...
def create_path_curve(model_path):
    ...
    for tendon_idx in range(model.ntendon):
        ...
        tendon_collection = get_or_create_collection("TENDON")
        tendon_collection.objects.link(curve_obj)
...
def import_mujoco_xml(filepath, mode='body', site_radius=0):
    ...
    def process_body(body, parent_transform=Matrix.Identity(4)):
        ...
        if mode == 'body':
            ...
            other_collection = get_or_create_collection("OTHER")
            other_collection.objects.link(body_empty)
...
if True:
    ...
    xml_file_path = "/home/micha/Desktop/mj_blender/mjmodel_locomotion.xml"
    ...
    # Organize remaining objects into collections
    tendon_collection = get_or_create_collection("TENDON")
    other_collection = get_or_create_collection("OTHER")
    support_collection = get_or_create_collection("SUPPORT")  # Create empty SUPPORT collection
    
    # Move only Camera and Light objects to OTHER collection
